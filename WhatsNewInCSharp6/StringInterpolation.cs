namespace WhatsNewInCSharp6
{
    public class StringInterpolation
    {
        ...
        {
            return string.Format("{0} er {1}", person.Name, adjective);
        }

        public string NewWay(Person person, string adjective)
        {
            return string.Format($"{person.Name} er {adjective}");
        }
        ...
        {
            return string.Format($"{person.Name} er {(person.Age > 32 ? "gammel" : "ung")}");
        }
    }
}